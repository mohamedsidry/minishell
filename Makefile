...
CC = cc
CFLAGS = -Wall -Werror -Wextra -Iminishell/include
CSTAGE = -c
...
UTILS = utils/general/nullstr.c  utils/general/ft_strcmp.c \
		...
		utils/general/ft_startwith.c utils/general/ft_itoa.c utils/general/ft_strtok.c \
...
EXECSRCS = utils/exec/executor.c \
           utils/exec/builtins.c \
           utils/exec/builtin_env.c \
           utils/exec/external.c \
           utils/exec/redirections.c \
           utils/exec/process.c \
           utils/exec/signals.c
...
SRCS = $(MAIN) $(UTILS) $(PARSING) $(LINKEDLISTSRC) $(ENVSRCS) $(VALIDATION) $(COMMANDSRC) $(EXECSRCS) $(TEST)

vpath %.h include
INCLUDES = header.h const.h error.h  test.h typedef.h utils.h exec.h
